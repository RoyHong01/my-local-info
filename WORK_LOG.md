...
## 2026-04-17 (보조금 블로그 중복 방지: 전화문의 기반 체크 추가)

- **문제 현상**: 범죄피해자 지원 관련 포스트가 무한 중복 생성됨. 동일 전화번호(1577-2584)를 사용하는 두 보조금 항목이 각각의 `source_id`로 존재하여 기존 중복 체크를 통과.
- **근본 원인**: `128000000001`(범죄 피해 구조금)과 `128000000003`(범죄피해자 경제적 지원)이 서로 다른 source_id이나 동일 기관(검찰청 피해자지원실, 1577-2584) 서비스.
- **수정 파일**: `scripts/generate-blog-post.js`
- **핵심 반영**:
  1. 보조금 카테고리에서 기존 포스트의 `source_id` → `전화문의` 번호 역추적 Set(`usedPhones`) 생성.
  2. 후보 필터에 "1.7순위: 전화문의 중복 체크" 추가 — 동일 전화번호 사용 시 후보 제외.
  3. 로그에 "전화문의 중복 체크: 기존 N개 번호 제외" 표시.
- **검증**:
  - 새 글 생성 시 범죄피해 대신 `수산업경영인육성사업` 선택됨.
  - `npm run build` 성공 (587페이지, 582 URL).
- **커밋**: `2c2cc61` — "fix: 보조금 중복 방지 전화문의 체크 추가 + 수산업경영인 글 생성"
