...
### GSC 실측 분석 → AdSense 신청 → 자동화 중단

**배경**

8/15 예정된 GSC 미색인 확인을 실행했다. 결과가 예상과 크게 달라
프로젝트 방향 자체를 재검토했다.
...
| --- | --- |
| 색인된 페이지 | **87** |
| 미색인 | 7,140 (404 5,008 / 크롤링됨-미색인 877 / 발견됨-미색인 992 / 기타) |
| sitemap URL | 1,700 |
| **3개월 총 클릭** | **1** |
| 3개월 총 노출 | 460 |
| 평균 CTR | 0.2% |
...
blog 1,047 내역: 일반 525 / 맛집 286 / 초이스 129 / 큐레이션 91 / 축제비교 16.
색인된 blog 23건 중 **맛집 14 + 초이스 6 = 20건**. 일반 블로그는 3건뿐이다.
...
**노출의 96%가 목록 페이지**다. blog 1,047개 중 노출이 잡힌 건 1건(3회).

**원인 분석**

1. **404 5,008건** — 4월 sitemap 8,298 URL 시대의 잔재.
   5/1 Top ID 축소(→1,432)로 사라진 ~6,900개를 Google이 재확인 중.
   `copilot-instructions` 규칙 21로 이미 "수용" 결정된 사항. 버그 아님.
2. **색인 811 → 87 붕괴 (5/28~6/02)** — sitemap 축소의 GSC 반영 지연.
   Google이 4주 뒤 대량 재평가한 결과. 같은 시점에 미색인이 774건으로 폭증.
3. **⚠️ blog sitemap 무필터** — `generate-sitemap.js:86 collectMarkdownPages()`는
   `posts`/`life`의 모든 `.md`를 조건 없이 수집한다.
   데이터 페이지(`collectIncheonPages` 등)에는 expired 제외 + description 비어있지
   않음 + `topIds.has(id)` 필터가 있으나 **블로그에는 없다.**
   7/09 sitemap Top 필터 보강도 인천·보조금·축제만 대상이었다. (**§2-10 패턴**)
4. **`validate-sitemap-subset.js`는 색인율을 보지 않는다.**
   "sitemap URL이 빌드 산출물에 존재하는가"만 검증한다.
   매일 통과했지만 색인 2.2%는 못 잡았다. (**§2-16 패턴**)

**판단**

sitemap 정리로 해결될 문제가 아니라고 결론지었다.

- 노출 460건은 **검색에서 사실상 존재하지 않는** 수준이다.
- sitemap을 100개로 줄여도 그 100개가 검색에 안 나오면 의미가 없다.
- 5개월 / 2,285페이지 / 투입 100만원+ → **3개월 검색 클릭 1회**.
- 콘텐츠 본질(공공데이터 재가공 + AI 생성)이 바뀌지 않는 한 개선 여지가 없다.
  원본이 정부24 등에 있는데 Google이 픽앤조이를 보여줄 이유가 없다.

**조치**

1. **AdSense 재신청 (8/15)** — 8/21로 예정돼 있었으나 6일 기다려도 달라질 게
   없어 즉시 신청. **목적은 통과가 아니라 반려 사유 확보**다.
2. **자동화 스케줄 중단** (`f50c121`) — `deploy.yml`의 `schedule` 트리거 주석 처리.
   `push` / `workflow_dispatch`는 유지.

**중단 효과**

| 항목 | 이전 | 이후 |
| --- | --- | --- |
| Anthropic API | 월 약 3만원 (9/1부터 4.5만원) | **0** |
| Gemini (상세 백필) | 미상 | **0** |
| 파일 수 증가 | 하루 45개 | **정지 (16,173 고정)** |

→ 9/1 단가 인상, 20,000 파일 한도, 호스팅 유료 전환($20/월) **세 문제가 동시에 소멸**.
사이트는 정적이라 그대로 서비스되며 Cloudflare 무료 한도 안이다.

**⛔ 취소된 예정 작업**

- 8월 말 Anthropic 예산 결정 (블로그 5→4건)
- 9월 중순 호스팅 결정 (Cloudflare Zone Pro vs Vercel)
- 9월 하순 상세 백필 소진 후 평시 2/2/5 복귀
- sitemap 블로그 필터 추가

**후속**

- AdSense 반려 사유 수령 후 `PROJECT_POSTMORTEM.md` 작성
- 이 데이터는 다음 프로젝트의 입력이다.
  `docs/NEW_PROJECT_IDEA_2026-08-11.md` §4에 실전 소재로 추가함.
...
축제 글 본문에 `<br>` 태그가 **화면에 텍스트 그대로 노출**되고 있었다.
축제 카테고리에서만, 1~3일에 한 번꼴로 발생했다.

### 원인(RCA)

- **렌더러가 `<br>` 을 지원하지 않는다.** `generate-curation-posts.js:460-475` 주석에
  이미 명시돼 있었다: *"ReactMarkdown에서 `<br>` 미지원이므로 `&nbsp;` 단락을
  여백용으로 사용"*. 큐레이션 쪽은 이 제약을 알고 우회하고 있었다.
- 그런데 블로그 프롬프트(`generate-blog-post.js:1853-1866`)는
  *"강제 줄바꿈: 최대 2~3문장마다 반드시 줄바꿈(Enter)할 것"* 만 지시하고
  **`<br>` 금지를 명시하지 않았다.** 모델이 이를 `<br>` 로 해석해 넣은 경우가 남았다.
- 매번이 아니라 간헐적으로 발생한 것은 **모델 출력 변동** 때문이다.
  코드 결함이면 매번 재현됐을 것이다.
- **§2-10 패턴**: 같은 제약(ReactMarkdown 의 `<br>` 미지원)을 큐레이션만 알고
  블로그 파이프라인은 몰랐다.
...
| 위치 | 건수 | 성격 |
| --- | --- | --- |
| 축제 2파일 | **9건** | **단독 줄** → 화면에 노출. 문제 |
| 인천·보조금 3파일 | 3건 | **마크다운 표 셀 내부** (`\| ... <br> ... \|`) → 정상 사용 |

총 12건 / 5파일. 실제 문제는 축제 9건뿐이었다.

### 조치

1. **기존 9건 제거** — 축제 2파일의 단독 `<br>` 줄만 삭제. 연속 빈 줄 정리.
   표 셀 3건은 **의도적으로 보존**.
2. **재발 방지 sanitizer 추가** (`generate-blog-post.js`)
   - `finalizeBlogRequest` 의 본문 확정 직후 1회 호출
   - **줄 전체가 `<br>` / `<br/>` / `<br />` 인 줄만** 제거 (공백 허용)
   - 줄 안에 다른 내용이 함께 있으면 건드리지 않음 → **표 셀 보존**
   - 제거 후 연속 빈 줄 3개 이상이면 2개로 축약

### ⛔ 맛집 방식을 그대로 쓰지 않은 이유

`generate-life-restaurant-posts.mjs:796-810` 에 이미 sanitizer 가 있으나
**전역 치환**(`/\s*<br\s*\/??\s*>\s*/gi` → `\n\n`)이라 표 셀 내 `<br>` 도 제거한다.
블로그(인천·보조금)에는 표가 있어 그대로 이식하면 표가 깨진다.
→ **단독 줄 전용 조건**으로 새로 작성했다.
...
- `16a2f0c` → main `78f62e1` fix(festival): remove standalone br tags from post bodies
- `ac3c964` → main `4660fa4` fix(blog): strip standalone br lines in finalize

### API 단가 확인 (2026-08-12)

| 모델 | 현재 | 9/1 이후 |
| --- | --- | --- |
| Sonnet 5 | $2/$10 (도입가, **8/31 종료**) | $3/$15 → **1.5배** |
| **Haiku 4.5** | $1/$5 | **변동 없음** |

- 배치 API 는 표준가의 50% 이므로 Sonnet 배치도 **1.5배** 오른다.
- 콘솔 실측(8/11): Sonnet `$0.69` + Haiku `$0.15` = `$0.85/일`
- 9/1 이후 예상: `$0.69×1.5 + $0.15 = $1.185/일` ≈ **1,640원**
- **현재 한도 1,500원을 초과한다.** 블로그 5 → 4건 축소 시 약 1,370원.
- **⚠️ 상세 백필(Gemini)이 9월 하순에 끝나도 Anthropic 비용은 줄지 않는다.**
  자연 해소되지 않으므로 **8월 말에 결정이 필요하다.**
...
- **80% 경고선(16,000) 도달.**
- 초이스·맛집 중단에도 증가폭이 줄지 않았다. 신규 수집 항목(8/12: 인천 1 /
  보조금 6 / 축제 4)의 상세 페이지가 블로그 감소분을 상쇄했기 때문이다.
  → **파일 수는 블로그 발행량이 아니라 원본 데이터 유입량에 좌우된다.**
- 한도(20,000) 도달 예상: 약 44일 → **9월 하순**

### 후속/주의

- **⚠️ 맛집 sanitizer 잠재 버그**: 전역 치환이라 맛집 글에 마크다운 표가
  생기면 표 셀의 `<br>` 이 제거되어 깨진다. 현재는 표가 없어 미발현. **미처리.**
- 블로그 프롬프트(`1853-1866`)에 `<br>` 금지 명시는 하지 않았다.
  sanitizer 가 구조적으로 막으므로 우선순위 낮음. **미처리.**
- 호스팅: **Cloudflare Zone Pro($20/월)는 선택지에서 제외**한다.
  파일 100,000 도달까지 약 2년 9개월로 유예를 사는 것일 뿐이고,
  같은 $20 이면 **Vercel Pro + ISR** 이 파일 수 개념 자체를 없앤다.
  9월 중순(파일 18,000 근처)에 AdSense 결과를 보고 결정한다.

--- 검증 ---

git diff --stat
git diff WORK_LOG.md

--- 중단 조건 ---

- 다른 파일이 변경되면 git restore 후 보고
- npm run build 는 실행하지 말 것 (문서 변경)
- 커밋·push 하지 말고 대기

## 2026-08-11

### 2026-09-01 Anthropic 단가 인상 (확인 완료)

- Sonnet 5 도입가($2/$10)가 2026-08-31 종료, 9/1부터 표준가($3/$15).
- **배치 API도 동일하게 1.5배 인상.** 배치 할인은 표준가 대비 50%이므로
  실효 단가가 $1.00/$5.00 → **$1.50/$7.50** 으로 오른다.
- 현재 일일 실제 1,016원 → 약 1,524원 예상. **한도 1,500원 초과.**
- 대응 후보: ① 예산 1,700원 상향 ② 발행량 5→3건(로드맵에 기존재)
  ③ 프롬프트 캐싱 강화. **8월 중 결정 필요.**

### 초이스·맛집 자동 생성 중단 (AdSense 대비)

**수정 파일**: `.github/workflows/deploy.yml`

**배경**
AdSense 재신청(8/21)을 앞두고 자체 품질 평가가 낮은 두 카테고리의
신규 발행을 중단했다. 기존 글은 삭제하지 않는다.

| 카테고리 | 중단 사유 |
|---|---|
| 맛집 | 방문한 적 없는 곳을 소개. 사진·평점이 Google Places 인용이라
Google이 원본을 알고 있다 |
| 초이스 | 쿠팡 제휴 링크 + 상품 나열. "제휴 마케팅 중심 사이트"로 보일 위험 |

**조치**
| 대상 | 방식 | 위치 |
|---|---|---|
| 초이스 | `CHOICE_AUTO_ENABLED: 'true'` → `'false'` | deploy.yml:465 |
| 맛집 후보 수집 | 기존 `if` 조건 앞에 `false &&` 추가 | deploy.yml:495 |
| 맛집 포스트 생성 | 기존 `if` 조건 앞에 `false &&` 추가 | deploy.yml:513 |

기존 조건식은 괄호로 감싸 그대로 보존했다.
스크립트 파일·기존 콘텐츠·커밋 step·리포트 step 은 건드리지 않았다.

**⭐ 복구 방법**
- 초이스: `CHOICE_AUTO_ENABLED` 를 `'true'` 로 되돌린다
- 맛집: 두 `if` 에서 **`false && ` 만 제거**한다 (기존 조건식은 그대로 남아 있음)

**유지한 것 (중단하지 않음)**
| 항목 | 하루 건수 | 이유 |
|---|---|---|
| 상세 markdown (인천·보조금·축제) | 15건 | 대기 695건. **9월 초 자연 종료**.
중단하면 상세 없는 항목이 색인에 남아 오히려 불리 |
| 블로그 (인천1·보조금2·축제1·큐레이션1) | 5건 | 품질 문제 없음.
심사 기간 활성도 유지가 유리 |

발행량: 하루 24건 → 20건.

**커밋**
- `45943c8` (작업 브랜치) / `eee2ae5` (main)
  chore(ci): pause choice and restaurant post generation

**검증 대기 (2026-08-12 리포트)**
- `🛍️ 초이스 포스트: 0건`
- `🍽️ 맛집 포스트: 0건`
- `♻️ 맛집 후보 재수집: 생략`
- `📝 블로그 변경: 5건` 유지 / `🧩 상세 markdown: 15건` 유지
- `💰 Anthropic API 비용` 감소
- 전체 `✅ 전체 정상 완료`

**미확인 사항**
리포트 스크립트가 skip 된 step 의 output 을 어떻게 처리하는지는
워크플로 파일에 명시돼 있지 않다. 리포트 생성이 실패하면
기본값 처리 추가가 필요하다.

**후속**
- 기존 초이스 글 요약문에 `"쿠팡 판매량 상위권 상품 중 중복 필터와
  품질 필터를 통과한..."` 내부 로직 문구가 노출된다. 목록 카드에도
  표시되므로 심사에 불리하다. **미처리.**
- 9월 초 상세 markdown 완료 시 비용 급감 예상. 그때 블로그 발행량을
  재조정하면 9/1 Anthropic 단가 1.5배 인상과 시기가 맞는다.

### 초이스 상업 신호 문구 제거 + 훅 대량 변경 경로 정비

**수정 파일**
- `scripts/generate-choice-posts-auto.js`
- `src/content/life/*` (초이스 글 100개)
- `scripts/check-worktree-safety.ps1`, `package.json`

**배경**
초이스 글의 요약문에 내부 선정 로직이 그대로 노출되고 있었다.
목록 카드에도 표시되므로 본문보다 노출 빈도가 높고, AdSense 심사에
상업 신호로 작용할 수 있어 8/21 신청 전에 정리했다.

> 기존: `<테마> 테마에서 쿠팡 판매량 상위권 상품 중 중복 필터와 품질 필터를
> 통과한 신규 아이템 3개를 엄선한 픽앤조이 초이스입니다.`

**원인(RCA)**
- `generate-choice-posts-auto.js:92` 의 **하드코딩된 payload summary 템플릿**.
  모델 프롬프트가 아니라 자동 생성 진입점에서 고정 문자열로 주입됐다.
  `${theme.name}` 만 치환되어 테마별 7종이 됐다.
- 이 값이 frontmatter 의 `summary` 와 `description` **양쪽에 동일하게** 저장됐고,
  본문 도입부에도 같은 문장이 독립 문단으로 삽입됐다.

**노출 경로**
| 위치 | 렌더 지점 |
|---|---|
| 블로그 목록 카드 | `BlogFilter.tsx:304` — `post.summary` |
| Life 목록 카드 | `life/page.tsx:78` 에서 summary→description 매핑 →
`LifeFilterClient.tsx:155` |
| 본문 | 도입 문단과 `오늘의 픽` 제목 사이 독립 문단 |

`posts.ts:208` 이 두 필드를 읽으며 description 누락 시 summary 를 사용한다.

**조치 — 3단계**

1. **생성기 차단** (`generate-choice-posts-auto.js:92`)
   `<테마> 테마에서 요즘 많이 찾는 아이템 3가지를 골라 특징과 활용법을 정리했어요.`
   로 교체. `${theme.name}` 치환 방식은 유지.

2. **frontmatter 100개 교체**
   임시 스크립트를 **파일로 작성**하고 `--dry-run` 으로 대상 100개를 확인한 뒤 실행.
   frontmatter 구간의 `summary`/`description` 2필드만 대상. 본문 무변경.
   결과: 잔여 매치 0건, 변경 파일 101개(콘텐츠 100 + 생성기 1).

3. **본문 정형 문장 77건 삭제**
   조사 결과 총 101건 / 87파일 / 고유 패턴 31종이었고, 성격이 둘로 갈렸다.

   | 유형 | 건수 | 형태 | 처리 |
   |---|---|---|---|
   | 정형 7종 | 77건 | **독립 문단 1줄** (앞뒤 빈 줄) | 삭제 |
   | 혼합형 24종 | 24건 | 도입·전환 문장에 섞임 | **유지** |

   정형 7종은 frontmatter 와 동일 문장이라 중복이었고, 독립 문단이라
   삭제해도 문맥이 끊기지 않았다. 삭제 후 도입부가 `오늘의 픽` 으로
   자연스럽게 이어지는 것을 실사이트에서 확인했다.

   혼합형 24종은 각각 다른 문장이고 `"에디터들이 직접 판매량과 품질 필터를
   꼼꼼히 따져"` 처럼 **선정 기준을 설명하는 자연스러운 문장**이라 유지했다.
   문제였던 것은 기계적으로 반복되는 템플릿 문장이었고 그것은 해소됐다.

   결과: 77파일 / 154줄 삭제(문장 77 + 중복 빈 줄 77) / 추가 0줄.
   HEAD 원문 대비 재현 검증으로 불일치 0 확인. build 2,255페이지 통과.

**⚠️ 훅이 정당한 작업을 세 번 차단 — 경로 정비**

100개 단위 콘텐츠 수정을 할 방법이 없어 `--no-verify` 가 반복될 상황이었다.
(8/06 WORK_LOG 에도 동일 패턴이 기록돼 있다.)

| 차단 | 원인 | 조치 |
|---|---|---|
| pre-commit | `check:worktree:commit` 이 `-MaxChanged 30` 을 **명시 전달** | package.json 에서 인자 제거 → 스크립트 기본값 10 + env override 동작 |
| pre-commit | 상한 자체 | `WORKTREE_MAX_CHANGED` env override 추가 (기본 10 유지, 명시적 `-MaxChanged` 최우선) |
| pre-push | Choice 이미지 가드가 변경된 초이스 글 전수 검사 | `WORKTREE_SKIP_CHOICE_IMAGE=1` skip 경로 추가 |

**⛔ 설계 원칙**: 기본값과 기존 검증 로직은 그대로 두고 **예외 경로만** 추가했다.
0바이트 감지 / FailOnDirty / `--diff-filter=d` / allowlist 는 무변경.

**검증**
- `WORKTREE_MAX_CHANGED=100`: 통과 / env=1 이어도 명시적 `-MaxChanged 100` 우선 /
  비정수 env 는 기본값 10 으로 회귀해 실패
-
...
### 배경
8/2 문서 정합화 작업 중 커밋 로그에서 날짜 이상을 발견했고, 이어서 사이트 육안 점검에서
hero 이미지 품질 문제와 맛집 미발행이 드러났다.

### 원인(RCA)
1. **날짜 UTC**: `generate-blog-post.js`의 `today`가 `toISOString()`(UTC) 기반.
   01:00 KST 실행이 UTC로는 전날이라 **파일명 slug와 frontmatter date가 하루씩 밀려** 발행됐다.
   `deploy.yml:56`은 이미 KST였으나 나머지가 UTC로 남아 있었다(§2-8).
2. **hero 이미지**: TourAPI `searchKeyword2`는 지역 필터가 없어 `'차이나타운'` 검색에
   **대림동(서울)** 사진이 잡혔고 인천 글 23건에 사용됐다. 등록 사진 품질 편차도 커
   상가 내부·전선 가림 등 hero 부적합이 다수였다(§2-9).
3. **맛집 미발행**: 수집 스크립트가 발행 이력을 보지 않아 7/29 수집 82건 중 **61건이
   수집 시점에 이미 발행된 식당**이었다. 실제 신규는 21건이라 6일 만에 소진됐다.
   리포트의 "재수집 생략"은 후보 충분이 아니라 **쿨다운**(7/29+7일) 때문이었다.

### 조치
- `deploy.yml` 커밋 메시지 5곳 → `TZ=Asia/Seoul` (`a9631d2`, `5973e05`).
  `generate-blog-post.js:1605` `today` → KST (`f6e56c1`). **소급 적용 없음**(URL 변경 방지).
- `BLOCKED_IMAGE_URLS` 신설 후 상가·부적합 사진 4종 8URL 차단
  (`84b45a3`, `d4304b9`, `c7df904`).
- **`CURATED_REGION_IMAGES` 화이트리스트 도입**(`7f6c131`). 사용자가 육안 승인한 인천 25장을
  1순위로 사용하고, 소진 시 기존 키워드 검색으로 폴백. 기존 로직 무손상.
- 보조금 4건(`0135eee`), 서울 자치구 27건(`57f7d13`), 시범 3건(`5717db4`),
  레거시 36건 + 중복 글 1건 삭제(`e1b1d3f`) 백필.
- 수집 단계에 발행 이력 제외 추가(`7a186b2`). guard가 `RESTAURANT_PUBLISHED_IDS` env로
  전달하고 collect가 구글 호출 **이전에** 제외 → 중복 방지 + Places 비용 절감.

### 검증
- 8/4 자동화: 파일명 `2026-08-04-`, 생성 시각 일치 → KST 정상.
- 8/4 신규 인천 글 hero가 큐레이션 목록(무의도자연휴양림)에서 나옴 → 자동화 경로 확인.
- 백필 후 차단 4종 잔여 0(서울분), `node --check` 및 `npm run build` 전 건 통과.

### 커밋
`a9631d2` `5973e05` `f6e56c1` `e834aed` `0135eee` `688b35d` `84b45a3` `d4304b9`
`c7df904` `57f7d13` `7f6c131` `5717db4` `bf6d4b3` `7a186b2` `e1b1d3f`

### 후속/주의
- **미해결**: `fix-post-images.js` 백필에서 `failed: 17`(전량 incheoneasy URL).
  `[WARN] pattern mismatch`만 찍히고 `newContent === content`가 되는 원인 미규명.
  정규식·frontmatter 바이트·캐시·`getRegionalLandmark` 반환값을 각각 확인했으나 모두 정상.
  **다음 조사는 추측이 아니라 `fix-post-images.js`에 임시 로그를 넣어 `newImage` 실제 값을
  찍는 것부터 시작할 것.** 잔여 29건.
- 8/5 맛집 재수집 예정(쿨다운 해제). 수집 건수가 크게 줄면 카카오 검색어 로테이션 확대 검토.
- 서울 등 타 지역 큐레이션 목록은 미등록. 인천 방식 그대로 확장 가능.
...
- **수정 파일**:
  ...
  - `COPILOT_MEMORY.md`
- **배경**:
  - 비교형 축제 포스트에서 카카오맵 버튼 아래 간격이 너무 붙어 보이고, `👉` 줄과 `에디터 성우의 한 줄 정리` 문구가 완성도를 떨어뜨렸음.
- **원인(RCA)**:
  - 생성기에서 카카오맵 링크를 손가락 이모지와 함께 직접 출력했고, 한 줄 정리도 너무 짧은 고정 템플릿으로 생성하고 있었음.
  - 상세 렌더러의 카카오맵 버튼 래퍼 하단 여백이 작아 다음 섹션과 시각적으로 덩어리처럼 붙어 보였음.
- **조치**:
  1. `buildVersusBody`에서 `👉` 없이 카카오맵 링크만 출력하도록 수정.
  2. `buildFinalGuide`의 제목을 `에디터의 한 줄 정리`로 바꾸고, 분위기·이동·체류감 기준이 드러나는 더 풍성한 요약으로 보강.
  3. 블로그 상세의 카카오맵 버튼 래퍼 하단 여백을 늘려 아래 문단과 분리감을 확보.
  4. 2026-05-16 비교형 축제 포스트를 현재 규칙에 맞게 직접 보정.
- **검증**:
  - `functions.get_errors` 확인.
  - `grep_search`로 `👉 [카카오맵 바로가기]`, `에디터 성우의 한 줄 정리` 잔존 여부 확인.
  - `npm run build` 실행.
...
- **Fix 1 — 보조금 카테고리명 오타 수정**:
  - `buildSubsidyItems()` 내 `category: '전국 보조금·복지'` → `'전국 보조금·복지 정책'`으로 통일
  - 블로그 포스트 카테고리명(`'전국 보조금·복지 정책'`)과 불일치가 있었음 → GSC 토픽 신호 혼재 방지
- **Fix 2 — 초이스 포스트 RSS description 개선**:
  - 자동 생성 초이스 포스트의 `summary`/`description` 필드가 내부 파이프라인 문구("XX 테마에서 쿠팡 판매량 상위권 상품 중...")로 고정되어 있었음
  - RSS에 그대로 노출되면 구글이 내용 없는 보일러플레이트로 평가할 수 있음
  - **RSS 렌더 시점 처리(Option A)**: `CHOICE_BOILERPLATE_RE` 정규식으로 보일러플레이트 탐지 → `"픽앤조이 초이스: {제목}. 품질·가성비를 검증한 쿠팡 추천 상품을 소개합니다."` 형태의 의미있는 설명으로 대체
  - 기존 게시된 모든 초이스 포스트에 즉시 적용 (생성 스크립트 변경 불필요)
- **빌드**: 성공 (1480 pages)
- **커밋**: 단일 커밋 + push
...
- **수정 파일**:
  - `scripts/generate-blog-post.js`
- **핵심 반영**:
  - 기존 일일 생성 구조는 유지하고, `전국 보조금·복지 정책` 카테고리에만 분석형 프롬프트 오버라이드를 추가.
  - 보조금 글에 아래 3개 필수 섹션을 강제:
    - `### 이런 분께 유리해요 (Best Fit)`
    - `### 신청 전에 준비할 3가지`
    - `### 비슷한 제도와 우선순위`
  - 허위 개인 경험 작성 방지 규칙 추가(확인되지 않은 체험/수치 단정 금지).
  - 생성 검증 함수 `hasSubsidyAnalysisStructure()` 추가:
    - 필수 섹션 누락 시 재시도, 최종 누락 시 저장 차단.
- **영향도**:
  - 데이터 파일(`subsidy.json`) 스키마 변경 없음.
  - 기존 축제/인천 생성 로직 및 일일 축제 생성 취지 유지(비교형 미도입).
- **빌드**: ✅ 성공 (1476 URLs, 8346건)
...
- **수정 파일**:
  ...
  - `scripts/generate-curation-posts.js`
- **핵심 반영**:
  - 현재 큐레이션 포스트 5개 서비스 소제목에 의미형 이모지 적용 (`🩺/🧩/🌾/🏠/🎗️`)
  - 큐레이션 자동 생성기 후처리에 `addEmojiToItemHeadings()` 추가
  - 생성 본문에서 항목 소제목(`###`, fallback `##`)에 카테고리별 이모지 풀을 순환 주입해 같은 글 내 반복을 줄이고 가독성 강화
  - 이미 이모지가 포함된 소제목은 중복 삽입하지 않도록 보호 로직(`hasLeadingEmoji`) 적용

## 2026-05-06 (축제 상세 페이지 UI 개선: Phase 4)

- **수정 파일**:
  - `src/app/festival/[id]/page.tsx`
- **핵심 반영**:
  - **카카오맵 길찾기 버튼 UI 추가**: `mapx`/`mapy` 좌표 기반 딥링크. 좌표 없으면 `addr1` 주소 검색 fallback. 노란 버튼 스타일(`bg-yellow-400`)로 헤더 하단에 배치
  - **"같은 지역 다른 축제" 카드 섹션 추가**: `lDongRegnCd` 우선(`areacode` fallback) 기준 동일 지역 필터링. 만료·종료 축제 제외, 시작일 정렬 후 최대 3개. 카드는 `grid-cols-1 md:grid-cols-3` 반응형
  - 서버 컴포넌트에서 `festival.json` 직접 필터링 — 클라이언트 JS 불필요
- **빌드**: ✅ 성공 (1476 pages, sitemap 1470 URLs)
- **커밋**: `014c334` — 단일 파일 단일 커밋
...
- **배경**: 광역 전용 정책 적용 후에도 지역명("서울"/"인천")으로 TourAPI를 검색하면 "지역명+시설명"으로 태깅된 노이즈 사진이 뽑힘. 사례:
  - "어선 장비 친환경 해양수산부" 글에 **서울둘레길 표지판** 사진 노출
  - "군포시 무료 독감" 글에 **안내판** 사진 노출
  - 사용자 지적: "단순한 사진을 갖다 쓰는 것이 아니고, **랜드마크**라는 게..."
- **해결**: 지역명 키워드 검색 → **큐레이션된 랜드마크명 풀**로 전환 + 주제 기반 매칭 도입.
- **신규 자료구조** (`scripts/lib/landmark-engine.js`):
  - `REGION_LANDMARKS`: 광역 17곳 × 4~7개 랜드마크 (서울→경복궁/남산서울타워/광화문/북촌한옥마을/청계천/덕수궁/창덕궁, 부산→해운대/광안대교/감천문화마을/태종대/용두산/오륙도, 제주→성산일출봉/한라산/협재해수욕장/만장굴/천지연폭포/용두암/우도, 경기→수원화성/에버랜드/임진각/광명동굴/남한산성/한국민속촌/두물머리 등)
  - `THEME_LANDMARKS`: 주제별 풀 (바다/어업/농업/산림)
  - `THEME_KEYWORD_MAP` + `detectThemeFromText()`: 어선·수산·해양수산부·해녀·양식 → 바다 풀, 농업·농민·농촌 → 농업 풀 등 자동 라우팅
- **검색 우선순위 변경** (`getRegionalLandmark`):
  1. theme 풀 (있으면)
  2. `REGION_LANDMARKS[광역]` 큐레이션 풀
  3. (폴백) 기존 지역명 키워드
  - 각 단계는 셔플로 다양성 확보
- **호출부 동기화**:
  - `scripts/fix-post-images.js`: 태그/요약/설명을 themeText로 묶어 전달, theme 감지 시 `__THEME__` 시도를 attempts 맨 앞에 삽입
  - `scripts/generate-blog-post.js`: 신규 자동 생성 글에도 동일하게 theme 감지 + curated pool 우선 적용
- **실행 결과**: `FORCE_RECHECK_ALL=1 node scripts/fix-post-images.js` → 67건 대상, **67건 전수 재교정 완료, 0 실패**
- **검증**:
  - 어선 글: 서울둘레길 표지판 → 바다 풀 매칭 사진 (`3534482`)
  - 군포 글: 안내판 → 경기 큐레이션 풀 사진 (`3563987`)
- **빌드/배포**: `npm run build` 성공 (8281 URL) → 커밋 `7f75d68` → push → Cloudflare 배포 진행 중

---

## 2026-04-23 (시·군 → 부모 광역도 매핑)

- **배경**: 광역 전용 정책에서, 본문에 광역명("경기" 등)이 빠지고 시·군만 있는 경우(예: "군포시 무료 독감") `군포` 단독으로 TourAPI 검색 → 노이즈 사진 매칭.
- **해결**: `CITY_TO_PROVINCE` 매핑 추가 (경기/강원/충북/충남/전북/전남/경북/경남 산하 약 90개 시·군).
- **로직**: `extractMetroFromText` 4순위 폴백에서 단독 시·군 발견 시 부모 광역도를 우선 push → `[광역, 시·군]` 순서로 반환.
- **충돌 키 의도적 제외**: 광주(광역시 vs 경기 광주시), 고성(강원 vs 경남), 군위(대구) 등.
- **검증**:
  - 군포시 → `[경기, 군포]`
  - 안동시 → `[경북, 안동]`
  - 창원시 → `[경남, 창원]`
...
## 2026-04-23 (랜드마크 정책 광역 전용 + 블루프린트 업데이트)

- **배경**: 1차 교정 후에도 인천 연수구 글에 의미 없는 테니스장 사진처럼, 구(區) 단위로 TourAPI를 검색하면 "지역명+시설명" 매칭으로 노이즈 사진이 뽑히는 문제가 남아있었음.
- **새 정책**: TourAPI 키워드는 **광역(시·도) 단위만** 사용, 구 단위는 모두 부모 광역으로 흡수.
  - 인천 연수구/부평구/중구 등 → `인천`
  - 서울 강북구/종로구 등 → `서울`
  - 경기 광주시 → `경기` (광주 모호성: 경기 컨텍스트면 `광주` 약칭 제외)
  - 광주광역시 → `광주`
  - 군산시/안동시 등 광역 정보 없는 단독 시·군 → 시·군 그대로 폴백
  - 경상남도 → `경남`, 충청남도 → `충남` 등 도 풀네임 → 약칭 매핑
- **변경 파일**:
  - `scripts/lib/landmark-engine.js`: `extractMetroFromText` 신설(광역 전용 추출), `PROVINCE_FULL_TO_SHORT` 매핑 추가
  - `scripts/fix-post-images.js`: `extractMetroFromText` 사용으로 단순화, `addressFilter`/`isDistrictToken` 로직 제거, `FORCE_RECHECK_ALL=1` 모드 추가(인천/보조금 카테고리 TourAPI 이미지 사용 글 일괄 재계산)
  - `scripts/generate-blog-post.js`: 신규 글 생성 경로도 동일하게 `extractMetroFromText` 사용으로 통일 (자동 생성 글에도 광역 정책 적용)
  - `AI_WEBSITE_BLUEPRINT.md`: F-3-1 (지역 랜드마크 자동 매칭), F-3-2 (맛집 네이버 API + 2장 구성) 신규 섹션 추가
- **실행 결과**: `FORCE_RECHECK_ALL=1 node scripts/fix-post-images.js` → 67건 대상, **66건 광역 정책 사진으로 재교정 완료**, 1건은 이미 동일 광역 사진이라 변경 없음.
