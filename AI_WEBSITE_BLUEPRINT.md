...
### A-7. 변경 범위 강제 차단 원칙 (Scope Guard)

지침 문서만으로는 AI/사람의 실수를 100% 막을 수 없다.
반드시 **git hook + preflight 스크립트**로 변경 범위를 기계적으로 차단한다.

#### 운영 규칙

1. 커밋 전 `.git/change-scope-allowlist.txt` 파일이 반드시 존재해야 한다.
2. 파일에는 이번 작업에서 허용할 경로/패턴만 한 줄씩 기록한다.
3. staged 파일이 허용 패턴 밖이면 pre-commit/pre-push에서 커밋/푸시를 차단한다.
4. 작업 종료 후에는 scope 파일을 비워(삭제) 다음 작업으로 넘기지 않는다.

#### 기본 명령

```bash
# 1) 이번 작업의 허용 범위 설정
npm run scope:set -- -Patterns "scripts/generate-curation-posts.js","src/content/posts/2026-05-08-curation-incheon.md"
...
- `scripts/check-worktree-safety.ps1`:
  - `-RequireScope` 모드에서 scope 파일 필수화
  - staged 파일이 허용 패턴과 불일치하면 즉시 실패
- `scripts/install-git-hooks.ps1`:
  - pre-commit / pre-push에서 scope 강제 검사를 항상 호출

> 이 원칙을 적용하면 "요청 범위 밖 임의 수정"은 커밋 단계에서 구조적으로 차단된다.

### A-8. 작업 기록 표준화 원칙 (Documentation & Memory Sync)

프로젝트가 커질수록 작업 이력, 현재 상태, AI용 압축 메모를 분리해야 한다.
문서 포맷이 흔들리면 나중에 원인 추적과 재현이 어려워지므로, 아래 기준을 모든 프로젝트에 공통 적용한다.

#### 기록 분리 원칙

- 상세 작업 이력은 `WORK_LOG.md` 같은 전용 로그 파일에 남긴다.
- 현재 상태와 핵심 마일스톤은 `PROJECT_MEMORY.md` 같은 요약 메모에 적는다.
- AI가 바로 참고할 압축 규칙은 `COPILOT_MEMORY.md` 같은 협업 메모에 둔다.
- 같은 내용을 중복 서술하지 말고, 역할에 따라 상세도만 다르게 유지한다.

#### 공통 작성 형식

- 모든 항목은 `수정 파일 → 배경 → 원인(RCA) → 조치 → 검증` 순서로 작성한다.
- 항목 제목은 `YYYY-MM-DD (작업명)`처럼 날짜 기반으로 통일한다.
- 조치 내용은 실제 변경만 적고, 추정이나 계획은 분리해서 쓴다.
- 검증에는 실행한 명령이나 통과 여부를 짧고 명확하게 적는다.
- 목록 중첩, 빈 줄, 헤딩 간격은 가능한 한 항상 동일한 스타일로 유지한다.

#### 운영 기준

- 새 작업을 끝낼 때는 코드 변경과 기록 갱신을 한 묶음으로 처리한다.
- 기록 파일만 먼저 고치고 코드 반영을 미루는 흐름은 금지한다.
- 기록을 바꿨다면 관련 메모 파일도 같은 배포 단위로 맞춘다.
- legacy 구간은 형식 예외를 허용하되, 신규 추가분은 반드시 공통 규칙을 준수한다.

#### 첫 페이지 저장 패턴

- 문서의 첫 페이지(상단)는 항상 `목적 → 적용 범위 → 작성 규칙 → 템플릿` 순서로 배치한다.
- 새 항목은 첫 페이지 아래에 날짜순으로 누적한다.
- 첫 페이지에는 예외 규칙과 운영 기준만 두고, 장황한 사례 설명은 뒤쪽 섹션으로 보낸다.
- 다른 웹사이트로 복사할 때도 상단 구조를 그대로 유지한다.

#### 특정 항목 수정 규칙

- 특정 파일이나 섹션을 수정할 때는 해당 문서가 이미 가진 제목/목록/들여쓰기 규칙을 우선 따른다.
- 관련 없는 항목을 함께 손대지 말고, 필요한 줄만 최소 수정한다.
- 기록 파일을 수정할 때는 코드 변경과 같은 작업 단위로 묶는다.
- 구조가 섞여 있으면 새 기준으로 무리하게 덧붙이지 말고, 기존 항목까지 같은 패턴으로 정리한 뒤 반영한다.

> 이 원칙은 다른 웹사이트 프로젝트에도 그대로 복사해 사용할 수 있는 공통 운영 규칙이다.
...
> ⚠️ **AdSense 심사 위험: CSR(fallback) 페이지 주의**
>
> Google AdSense 심사봇은 구글 검색봇과 달리 JavaScript 실행 결과를 기다리지 않는다.
> `generateStaticParams`에 포함되지 않은 `/view?id=...` CSR 페이지 또는 `fallback: true`로
> 처리되는 동적 라우트는 심사봇 기준 **"로딩 스피너만 있는 빈 페이지"**로 인식된다.
>
> - CSR 페이지가 sitemap.xml에 포함되면 AdSense 심사에서 "Low Value Content" 판정 위험이 증가한다.
> - **대응책**: sitemap에는 SSG 페이지만 포함. CSR 라우트는 `<meta name="robots" content="noindex">`
>   또는 robots.txt `Disallow`로 처리. 레거시 라우트(`/view?id=...`)는 SSG 정합 완료 후 제거.
...
#### H-5-1. "Low Value Content" 거절 대응 플랜 (실전 교훈)

> **AdSense ≠ SEO**: 구글 검색봇은 CSR 페이지도 어느 정도 처리하지만,  
> **AdSense 심사봇은 "광고주에게 가치 있는가"를 판단하며, JS 실행을 기다리지 않는다.**

**"Low Value Content" 판정 주요 원인 5가지**:

| # | 원인 | 증상 | 대응 |
|---|------|------|------|
| 1 | fallback 페이지 = CSR | 심사봇이 "로딩 중..." 텍스트만 봄 | SSG 전환 또는 noindex 처리 |
| 2 | 공공 API 데이터 나열 | 독창성·편집 가치 부재로 판정 | editor_note + 큐레이션 레이어 추가 |
| 3 | sitemap에 CSR URL 포함 | 색인 시도 → "빈 페이지" 반복 신호 | sitemap을 SSG 페이지만으로 제한 |
| 4 | 레거시 라우트 잔존 | `/view?id=...` 같은 저가치 URL 노이즈 | 301 redirect 또는 삭제 후 _redirects 처리 |
| 5 | 편집적 가치 부재 | 데이터 나열만으로 독자 이익 없음 | AI tip/주석/큐레이션 레이어로 보강 |
...
```bash
# 1. 레거시 라우트 제거
# src/app/subsidy/view/page.tsx → 삭제 후 _redirects에 추가
/subsidy/view  /subsidy  301

# 2. sitemap을 SSG 페이지만으로 제한
# scripts/generate-sitemap.js에서 CSR 라우트 URL 제외
# generateStaticParams에 포함되지 않은 동적 경로는 sitemap 배제

# 3. CSR 라우트에 noindex 태그 추가
export const metadata = { robots: { index: false } };

# 4. RSS 피드와 sitemap URL 수 정합 확인
# RSS에 있는 포스트가 sitemap에도 있는지 크로스체크
```

**Phase 2 — 콘텐츠 품질 보강 (3~7일)**:
```
① editor_note 자동화:
   - Claude Haiku로 데이터 항목마다 tip 3가지 자동 생성
   - 상세 페이지 하단 "에디터 노트" 콜아웃 박스 UI

② 큐레이션 블로그 자동화:
   - 요일별 카테고리 TOP-N 집계형 포스트 (글쓰기 앵글 4종 로테이션)
   - category: 큐레이션 으로 블로그 목록에 노출

③ 홈 "이번 주 픽" 섹션:
   - 큐레이션 포스트 최신 3개를 메인에 노출
   - 없으면 graceful null (항상 노출 강제 금지)
```

**Phase 3 — 재신청 (Phase 2 완료 후)**:
```bash
# 1. 심사봇 기준 페이지 정상 노출 확인
curl -A "Googlebot" "https://example.com/blog/some-post/" | grep -c "<h1"

# 2. Search Console → URL 검사 → 색인 재요청 (주요 페이지 5~10개)

# 3. 1~2주 추이 확인 후 AdSense 검토 요청
#    (Search Console에서 빨간 오류 0건 확인 후 신청)

# 4. 심사 기간 동안 Cloudflare Bot Fight Mode OFF 유지
```

**실전 교훈 (픽앤조이 2026-04 경험)**:
- fallback 페이지가 sitemap에 포함되어 있어 색인 7,500개 중 실질 노출 불명확
- 레거시 `/subsidy/view` 라우트 제거 + sitemap SSG 정합 후 Search Console 재제출
- editor_note + 큐레이션 블로그 추가로 "편집 가치 없음" 대응
- Phase 1+2 완료까지 약 1주일 소요
...
> ⚠️ **유료 전환 전 반드시 현재 사용량 확인**:
> ```bash
> wrangler pages project list  # 현재 배포 파일 수 확인
> ```
> 사용량이 **15,000개 미만**이면 유료($20/월) 전환은 무의미하다.
> 배포 실패 원인이 "파일 수 초과"가 맞는지 먼저 확인하고, 아니라면 다른 원인(빌드 오류 등)을 찾는다.

> ⚠️ **AdSense × 하이브리드 충돌 주의**:
> SSG → 하이브리드(SSG + fallback CSR) 전환 후 AdSense 심사를 신청하면 "Low Value Content" 판정 위험이 생긴다.
> 반드시 아래 세 값이 일치하는지 확인 후 심사 신청할 것:
> ```
> sitemap URL 수  =  generateStaticParams 반환 페이지 수  =  실제 SSG 빌드 페이지 수
> ```
> 하이브리드 전환 시 sitemap에서 CSR 라우트 URL을 반드시 제거한다.
...
| 9 | AdSense "Low Value Content" 거절 | fallback(CSR) 페이지가 sitemap에 포함됨 | Phase 1~3 플랜 (H-5 참조) |
| 10 | sitemap URL > SSG 페이지 수 | generateStaticParams 누락 또는 레거시 라우트 잔존 | sitemap SSG URL만 포함 + _redirects 처리 |
| 11 | 공공 API 데이터 = Low Value 판정 | 편집적 가치 없는 데이터 나열 | editor_note + 큐레이션 블로그 레이어 추가 |
...
### K-14. editor_note 자동화 패턴 (공공 데이터 가치 보강)

**목적**: 공공 API 데이터(인천시 정보/보조금/축제 등)는 기계적 나열에 불과해 AdSense 심사에서 "Low Value Content"로 판정될 수 있다. `editor_note`는 각 데이터 항목에 AI가 생성한 큐레이터 관점의 팁 3가지를 추가해 편집적 가치를 보강하는 패턴이다.
...
```json
{
  ...
  "title": "인천 가정의달 행사",
  "editor_note": [
    "신청 마감 2주 전부터 신청 경쟁이 치열해지므로 일정 공개 직후 바로 접수하세요.",
    "무료 입장이지만 주차 공간이 협소하므로 대중교통 이용을 권장합니다.",
    "가족 단위 방문객을 위한 포토존이 잘 갖춰져 있어 기념 사진 찍기 좋습니다."
  ]
}
```

**생성 스크립트 패턴** (`scripts/generate-editor-notes.js`):
```javascript
// Claude Haiku로 배치 처리 — 항목당 1회 호출, 기존 editor_note가 있으면 스킵
const BATCH_SIZE = parseInt(process.env.EDITOR_NOTE_BATCH_SIZE || '10');

// 우선순위: 마감 임박 > 조회수 높은 항목 > 최근 수정 항목 순
const candidates = items
  .filter(item => !item.expired && !item.editor_note)
  .sort((a, b) => /* 마감임박 정렬 */ ...)
  .slice(0, BATCH_SIZE);

for (const item of candidates) {
  const note = await generateEditorNote(item);  // Claude Haiku 호출
  item.editor_note = note;
}
```

**UI 패턴** (상세 페이지 하단 카테고리별 컬러 콜아웃 박스):
```
인천: 파란색 (bg-blue-50, border-blue-200)
보조금: 황색  (bg-yellow-50, border-yellow-200)
축제:  장미색 (bg-rose-50, border-rose-200)

헤딩: "📌 픽앤조이 큐레이터의 한 마디"
내용: ul 리스트로 tip 3가지 노출
```
...
# .github/workflows/deploy.yml
- name: 에디터 노트 생성
  run: node scripts/generate-editor-notes.js
  continue-on-error: true  # 생성 실패가 전체 배포를 막지 않도록
  env:
    ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
    EDITOR_NOTE_BATCH_SIZE: 10
```

**주의**:
- `continue-on-error: true`는 필수 — 단건 API 오류가 전체 파이프라인을 막지 않도록
- 기존 `editor_note`는 덮어쓰지 않음 (생성 비용 절감)
- 생성 후 데이터 JSON을 커밋해야 다음 실행 시 스킵됨 (단계적 누적 방식)

---

### K-15. 큐레이션 포스트 자동 생성 & AI 패턴 방지

**목적**: 공공 API 데이터를 그대로 나열하는 단건 포스트 외에, 카테고리 TOP-N 집계형 큐레이션 포스트를 추가하면 "편집자가 선별했다"는 가치를 구글/AdSense에 전달할 수 있다.

**슬러그 패턴**: `YYYY-MM-DD-curation-{짧은-키워드}`  
**frontmatter 예시**:
```yaml
---
title: "이번 주 인천 행사 TOP 5 — 가족 나들이 추천"
date: 2026-05-02
category: 큐레이션
tags: [인천, 주간추천]
published_by: auto
...
**요일별 카테고리 테마 예시**:
```
월: 보조금 (이번 주 마감 임박 보조금 TOP 5)
화: 인천 (이번 주 인천 행사 TOP 5)
수: 축제 (이번 주말 전국 축제 TOP 5)
목: 보조금 (연령/직군별 보조금 큐레이션)
금: 인천 + 축제 (이번 주말 인천 근교 여행 코스)
```

**글쓰기 앵글 4종 로테이션 (AI 패턴 방지)**:
```javascript
// 날짜 시드 기반으로 매일 다른 앵글 사용 → 제목/서론 패턴 반복 차단
const ANGLES = [
  '문제_해결형',    // "마감 임박! 놓치면 아쉬운 보조금 5가지"
  '트렌드_중심형',  // "이번 주 인기 급상승 인천 행사"
  '전문가_큐레이션', // "픽앤조이 에디터가 직접 고른 주말 축제"
  '가성비_강조형',  // "무료 혹은 저렴한 이번 주 문화 행사"
];
const angle = ANGLES[new Date().getDate() % ANGLES.length];
```
...
const curationPosts = getSortedPostsData()
  .filter(p => p.category === '큐레이션')
  .slice(0, 3);  // 최신 3개만

// curationPosts가 0건이면 섹션 자체를 렌더링하지 않음 (graceful null)
{curationPosts.length > 0 && (
  <section>
    <h2>이번 주 픽</h2>
    {curationPosts.map(post => <PostCard key={post.slug} post={post} />)}
  </section>
)}
```
...
```yaml
- name: 큐레이션 포스트 생성
  run: node scripts/generate-curation-posts.js
...
**금칙어 (제목/서론 반복 방지)**:
```
"총정리", "완전정복", "한눈에", "이것만 알면", "결론적으로",
"다양한", "추천드려요", "모아봤어요", "알아보겠습니다"
```
