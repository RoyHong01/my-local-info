...
1. **특정 파일 단독 수정 원칙**: 사용자가 특정 파일을 지정해서 수정을 요청하면, **오직 그 파일 하나만** 수정한다. 관련이 있어 보이더라도 다른 파일을 함께 수정하지 않는다.
2. **불명확 시 무조건 질문**: 수정 대상이나 요청 의도가 조금이라도 불확실하면, 임의로 판단해서 진행하지 말고 반드시 사용자에게 먼저 질문하고 확인받은 후 작업을 시작한다.
3. **공통 파일 수정 금지**: `globals.css`, `layout.tsx` 등 공통 파일은 사용자가 명시적으로 지정하지 않는 한 절대 수정하지 않는다.
4. **확장 수정 사전 허락 원칙**: 요청 범위 밖의 파일을 수정해야 할 필요가 생기는 순간, 작업을 즉시 멈춘다. 설령 도움이 된다고 판단되더라도, **반드시 먼저 사용자에게 이유와 영향 파일 목록을 설명하고 허락을 받은 후에만 실행한다. 허락 전에는 어떤 파일도 수정하지 않는다.**
...
- **버그 수정 프로토콜 (Bug Fix Protocol — 필수)**: 버그를 즉각 수정한 직후, 반드시 다음 3단계를 연속으로 수행한다. "빌드 성공"이 나와도 이 단계를 건너뛰지 않는다.
...
## 작업 규칙
1. **[필수] 작업/디버깅 시작 전 `PROJECT_CRITICAL_NOTES.md`를 먼저 읽는다. 특히 '알려진 함정(§2)'을 확인해 이미 규명된 원인을 처음부터 다시 추론하지 않는다. 진단이 과거 사실이나 사용자 반례와 충돌하면 그 진단을 폐기하고 재조사한다.**
2. 작업 전 이 파일과 `AGENTS.md`, `COPILOT_MEMORY.md`를 먼저 확인
3. 세부 작업 이력은 `WORK_LOG.md`에 기록
4. Copilot 운영 문서는 `.github/copilot-instructions.md`와 `COPILOT_MEMORY.md`를 기준으로 유지
5. `npm run build` 항상 마지막에 실행해서 빌드 오류 확인
6. **과금 API 실호출 금지 규칙(상시)**: 과금 API(Google Places 등) 실호출은 검증·테스트 목적 사용 금지. 검증은 mock/시드 캐시/정기 런 리포트 수치로 대체한다. 실호출이 불가피한 경우 사전에 사용자 승인 필수.
7. 빌드 성공 후 `git add . → git commit → git push` 순서로 배포
8. 세션 종료 시 `WORK_LOG.md`, `COPILOT_MEMORY.md`, `.github/copilot-instructions.md` 업데이트
9. **항상 적용할 종료 루틴(필수, 단일 호흡 배포)**
...
8. **단일 커밋·단일 푸시 원칙(필수)**: 코드 커밋과 문서 동기화 커밋을 분리하지 않는다. "코드 push → 별도 docs sync push" 패턴은 금지. 이유: 연속 push 시 GitHub Actions가 이전 run을 자동 취소(`Canceling since a higher priority...`)하면서 빨간 X/⚠️가 누적되고, 빌드 시간도 낭비된다.
  ...
  - 사용자에게 별도 확인 질문 없이 자동으로 단일 커밋으로 묶는다.
    - **파일 손상 방지 규칙과의 관계**: 단일 커밋 원칙은 '커밋·push 시점'에만 적용된다. 수정 자체는 반드시 한 파일씩 순차 진행한다(수정 → 문법/빌드 검증 → `git diff --stat` 확인 → 다음 파일). 여러 파일을 한 번에 str_replace하라는 의미가 아니다. (`PROJECT_CRITICAL_NOTES.md` §3-3 참조)
...
18. **단독 초이스 생성 경로 통일 규칙(필수)**:
  ...
  - 실행은 반드시 `npm run generate:choice:request -- --input scripts/choice-input.<요청명>.json` 경로를 사용한다.
    - 내부적으로 `choice-input.latest.json`을 임시 교체해 `generate:choice:latest -> check:choice-quality -> build` 체인을 실행한 뒤 자동 원복한다.
  - `scripts/choice-input.latest.json`은 baseline 파일로 취급하며, 기본적으로 커밋하지 않는다(훅에서 기본 차단).
  - 위 체인을 통과하지 않은 단독 포스트는 완료본으로 간주하지 않는다.
  - 목적: 자동/단독 품질 편차 제거, 동일 프롬프트·검증 체인 적용, 재발 이슈 RCA 단순화.
19. **오늘 포스팅 조회 시 원격 우선 동기화 규칙(필수)**:
  - 픽앤조이 자동화 파이프라인은 매일 **01:00 KST**(`0 16 * * *`)에 실행되어 GitHub에 push하고 Cloudflare Pages에 배포된다. *(2026-07-24 Anthropic Batch 도입에 맞춰 04:00 → 01:00 변경)*
  - 사용자가 **당일(오늘) 생성된 포스트/파일**에 대해 질문하면(예: "오늘 만들어진 블로그", "오늘 자동생성된 포스트"), **로컬 파일을 검색하기 전에 반드시 먼저 `git pull`을 실행**해 원격과 동기화한다.
...
- 2026-05-03 핵심 반영(큐레이션+축제 품질 보강):
  ...
  - **실제 오류 포스트 교정**: `src/content/posts/2026-05-02-GangneungDanoje.md`의 source 메타·이미지·한눈에 보는 정보·주소/길찾기·같은 지역 링크를 강릉단오제 데이터(`source_id: 531391`)로 복구.
...
- 2026-04-22 핵심 반영(추가):
  ...
  - **단독 생성 경로 완전 통일**: 단독 요청은 `generate:choice:request -- --input scripts/choice-input.<요청명>.json`으로만 처리하고, `choice-input.latest.json`은 자동 원복하도록 운영 규칙 고정.
  ...
  - **단독 초이스 포스트 추가 생성**: `src/content/life/2026-04-22-choice-latex-handska-powerfree-s-set.md`.
  - **쿠팡 연동 반영**: `https://link.coupang.com/a/eug11e`와 배너 이미지를 frontmatter에 반영.
  - **이미지 자산 반영**: `public/images/choice/latex-hero.png`, `public/images/choice/latex-middle.png` 추가.
  - **본문 톤 유지**: 일상 시나리오 중심, 과장 금지, 자연스러운 CTA 정책으로 작성.
  - **단독 초이스 포스트 추가 생성**: `src/content/life/2026-04-22-choice-holika-letter-from-spring.md`.
  - **쿠팡 연동 반영**: `https://link.coupang.com/a/euje3w`와 배너 이미지를 frontmatter에 반영.
  - **이미지 자산 반영**: `public/images/choice/holy-hero.png`, `public/images/choice/holy-middle.png` 추가.
  - **본문 톤 유지**: 일상 시나리오 중심, 과장 금지, 자연스러운 CTA 정책으로 작성.
- 2026-04-21 핵심 반영(추가):
...
- 2026-04-03 핵심 반영:
  - GitHub Actions 스케줄 04:00 KST로 변경 (`0 19 * * *`) *(→ 2026-07-24 Anthropic Batch 도입으로 01:00 KST `0 16 * * *`로 재변경)*
