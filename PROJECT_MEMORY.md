...
## 최근 마일스톤 (2026-07-24, Sonnet 5 블로그 통합 Batch 전환)

- 배경:
  - 일반 블로그·요일별 큐레이션·축제 비교 생성의 모델/비용/실패 관측을 하나의 운영 경로로 통합할 필요가 있었음.
- 원인:
  - 세 생성기가 독립 모델 호출 구조를 가져 Batch 할인, 부분 실패 회수, 일일 통합 예산 차단을 공통 적용하기 어려웠음.
- 조치:
  - 블라인드 A/B 결과로 `claude-sonnet-5`를 채택하고, 3개 생성기를 prepare/finalize 구조로 분리.
  - 공통 러너가 초기 요청을 단일 Anthropic Batch로 제출하고 20초 폴링/30분 타임아웃 후 부분 결과를 회수.
  - unresolved 요청만 동기 fallback하며, 일일 1,500원/80% 경고 가드가 예상 초과 호출을 사전 차단.
  - 예산 중단 항목은 `warning_budget_stop` 미발행 처리하고 Batch/비용/fallback/미발행 지표를 리포트·텔레그램에 연결.
  - 기존 프롬프트/검증/후처리 및 초이스·맛집 Gemini 경로는 유지.
- 검증:
  - fake-client 회귀 테스트, 프롬프트 해시 3종 고정, 구문/진단, 전체 빌드 통과.
...
- 원인:
  - Top ID 기반 SSG/RSS/sitemap 정합은 AdSense 대응 핵심인데, 404 완화와 섞여 경계가 흐려질 여지가 있었음.
- 조치:
  - `.github/copilot-instructions.md`에 `SSG 상한/색인 안정화 정책` 추가.
  - `generateStaticParams`/`sitemap`/`RSS`는 Top ID 기준 유지, 상한 해제는 사용자 명시 승인 필수로 고정.
  - 404 완화 기본 해법은 수집 스크립트에서 과거 데이터를 삭제하지 않고 `expired`로 보존하는 방식으로 명시.
- 검증:
  - WORK_LOG/PROJECT_MEMORY/COPILOT_MEMORY 간 동일 정책 반영 확인.

## 최근 마일스톤 (2026-05-18, festival-versus 카카오맵 간격 및 한 줄 정리 품질 보강)

- 배경: 비교형 축제 포스트에서 카카오맵 버튼과 다음 문단이 붙어 보이고, `에디터 성우의 한 줄 정리` 문구와 요약 밀도가 아쉬웠음.
- 원인:
  - 생성기가 `👉 [카카오맵 바로가기]`를 직접 출력하고, 한 줄 정리를 짧은 고정 문장으로 생성하고 있었음.
  - 상세 렌더러의 카카오맵 링크 버튼 하단 여백이 작아 다음 섹션과 분리감이 약했음.
- 조치:
  - `scripts/generate-festival-versus-post.js`에서 손가락 이모지를 제거하고, `에디터의 한 줄 정리`를 분위기·이동·체류감이 드러나는 더 풍성한 요약으로 보강.
  - `src/app/blog/[slug]/page.tsx`에서 카카오맵 버튼 하단 여백을 확대.
  - 기존 발행본 `2026-05-16-festival-versus-gongju-daehakro.md`도 같은 규칙으로 보정.
- 검증:
  - `functions.get_errors` 확인.
  - `npm run build` 성공.

## 최근 마일스톤 (2026-05-18, 전국 보조금 포스트 Best Fit 문구 제거)

- 배경: 보조금 글의 섹션 제목에 `Best Fit` 영문 표기가 반복되어 스타일을 한글로 통일해야 했음.
- 원인: `scripts/generate-blog-post.js`의 subsidy 전용 프롬프트가 영문 표기를 직접 지시하고 있었음.
- 조치:
  - 프롬프트 제목을 `이런 분들께 유리해요`로 변경.
  - 2026-05-03 이후 생성된 보조금 포스트 30건의 헤딩을 일괄 정리.
- 검증:
  - `node scripts/replace-best-fit-headings.mjs` 실행 후 30건 갱신 확인.
...
- 배경: 비교형 축제 포스트 히어로가 300x200 썸네일로 선택되어 시각 품질 저하 발생.
- 원인:
  - 히어로 이미지 선택이 품질 정보 없이 랜덤 기반으로 동작.
  - 본문 이미지와 히어로 이미지 분리가 약해 동일 계열 이미지 중 저품질 선택 가능성 존재.
- 조치:
  - `scripts/generate-festival-versus-post.js`에 품질 점수 기반 선택 로직 도입.
  - 히어로는 상위 품질군에서만 선택하고, 본문은 히어로 동일 URL을 우선 배제.
  - `src/content/posts/2026-05-09-festival-versus-daejeon-busan.md`의 히어로 URL을 고해상도 이미지로 교체.
- 검증:
  - `npm run build` 성공

## 최근 마일스톤 (2026-05-09, festival-versus 본문 이미지 품질 복구)

### 완료: 히어로 분리 과정에서 발생한 본문 저화질 회귀 수정

- 배경: 히어로와 다른 이미지를 사용하도록 조정하던 중 본문 첫 축제 이미지가 저화질 썸네일로 교체됨.
- 원인:
  - 본문 이미지 선택이 "히어로와 다른 URL"을 과도하게 우선해 고해상도 원본보다 저화질 대체 이미지를 채택함.
- 조치:
  - `scripts/generate-festival-versus-post.js`에서 본문은 가장 품질이 좋은 이미지를 기본 유지하도록 변경.
  - 대체 이미지 품질이 충분히 좋을 때만 히어로와 분리하도록 가드 추가.
  - `src/content/posts/2026-05-09-festival-versus-daejeon-busan.md` 본문 이미지를 고해상도 URL로 복구.
- 검증:
  - `npm run build` 성공
...
### 완료: Supabase 로그 정리 + 맛집 서론 규칙 재고정

```bash
커밋 8592c46 - Supabase 경고 1회 출력 + 맛집 후처리 검증 로그 개선
커밋 3b93b53 - 첫 소제목 전 150자 제한 완전 제거
커밋 74340ff - 첫 소제목 전 서론 규칙을 2~3줄 -> 2~3문단으로 고정
```

### 핵심 운영 의미

1. Supabase는 비용 절감을 위한 핵심 캐시 경로로 유지
2. 연결 불안정 시에도 Google API 폴백은 유지
3. 반복 경고는 1회만 출력해 로그 노이즈 제거
4. 맛집 서론은 문자수 상한 없이 자연스러운 2~3문단을 기본값으로 고정

### 현재 고정 기준

- 첫 소제목 전 브릿지 서론: 2~3개 문단
- 150자 제한: 미적용
- 2개 미만/3개 초과 브릿지 문단: 검증 실패

### 추가 반영 (2026-04-21)

- 픽앤조이 초이스 단독 제품 포스트 1건 생성: 펩시 제로 슈거 라임향 500ml 24입
- 히어로 이미지: `public/images/choice/pepsi-hero.png` (다운로드 소스 반영)
...
- 수집 시 markdown 배치 생성:
  ...
  - 축제: 2
  - 보조금: 5
- 인천 이미지 정책: 인천관광공사 API는 비활성화하고 TourAPI 키워드 매칭 성공 시에만 이미지 사용
...
- 인천관광공사 API 경고 라인은 정책 전환 후 기본 비노출
