...
- **수동 블로그 3편 추가 (벚꽃 시리즈):**
  - 진해 군항제: "지금 당장 떠나야 한다"는 긴박감 + 현재 진행 강조
  - 여의도 봄꽃: D-5 카운트다운 + 서울 접근성 + 치맥 피크닉 감성
  - 경포 벚꽃: 동해 바다 + 벚꽃 유일한 차별점 + 강릉 커피 연계 코스
  - 누적 블로그 포스트: 총 20편
- **자동 블로그 생성 MZ 감성 스타일 업그레이드:**
  - 첫 문장 짧고 강렬하게 (D-5, 솔직히 말할게요 등)
...
- **GitHub Actions 자동 생성 설정 확인:**
  - `generate-blog-post.js`: 카테고리당 2편 생성 (`generated >= 2` break) 정상 확인

### 2026-03-27 (14)

- **광안리 드론라이트쇼 공식 포스터 이미지 저장 및 배포:**
  - `public/images/gwangan-drone-2026-poster.png` 저장 (686KB, 다운로드 폴더에서 복사)
  - `2026-03-26-gwangan-drone-lightshow.md` image 필드: TourAPI URL → 로컬 공식 포스터

### 2026-03-27 (13)

- **광안리 드론라이트쇼 블로그 썸네일 교체 배포:**
  - `2026-03-26-gwangan-drone-lightshow.md` image 필드: 기존 → TourAPI firstimage3 실제 드론쇼 사진
...
- **블로그 이미지 지침 강화:**
  - 벚꽃/산수유 포스트 썸네일 이미지 주제 관련 이미지로 교체 배포
  - `generate-blog-post.js` 프롬프트에 이미지 주의사항 추가 (주제 무관 이미지 절대 금지)
  - `CLAUDE.md` 작업 규칙 10번: 수동 블로그 이미지 규칙 추가 (Unsplash 영문 검색 가이드 포함)
