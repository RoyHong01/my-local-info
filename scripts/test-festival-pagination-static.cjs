...
async function getCount(page) {
  return page.locator('[data-testid^="festival-card-"]').count();
}
...
async function clickLoadMoreTo(page, targetCount) {
  for (let i = 0; i < 6; i += 1) {
    ...
    const button = page.getByTestId('festival-load-more');
...
async function run() {
  ...
  try {
    await page.goto(`${BASE_URL}/festival/`, { waitUntil: 'domcontentloaded' });
    await page.waitForSelector('[data-testid^="festival-card-"]', { timeout: 10000 });
    ...
    await page.locator('[data-testid^="festival-card-"]').nth(89).click();
    await page.waitForURL((url) => url.pathname.startsWith('/festival/') && url.pathname !== '/festival/', { timeout: 10000 });

    const savedScroll = Number(await page.evaluate(() => sessionStorage.getItem('festivalScrollY') || '0'));

    await page.goBack();
    await page.waitForURL((url) => url.pathname === '/festival/', { timeout: 10000 });
    ...
    console.log('[PASS] Festival static pagination checks passed');
  } finally {
    await browser.close();
  }
}
...
run().catch((error) => {
  console.error('[FAIL] Festival static pagination checks failed');
