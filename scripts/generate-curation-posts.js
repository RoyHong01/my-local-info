...
function summarizeItem(item, category, index, ssgEligibleIds) {
  ...
  const detailUrl = buildPolicySafeDetailUrl(category, item, { [category]: ssgEligibleIds });
...
5. 각 항목은 반드시 \`### [소제목]\` 형식의 소제목 헤딩으로 시작하세요. 번호 라벨(1. 2. 3.) 금지. 독자가 읽고 싶어지는 감성형·매거진형 소제목을 자유롭게 작성하세요. (예: "### 매달 나가는 교육비, 이렇게 돌려받으세요")
6. 각 항목 설명은 3~5문장: 혜택 금액, 지원 대상, 신청 방법, 신청 기한 포함. 딱딱하지 않게.
7. 각 항목 설명이 끝나면 반드시 빈 줄을 한 줄 넣고, 그 다음 줄에 \`[자세히 보기](URL)\` 링크를 단독으로 배치하세요. 링크 뒤에도 빈 줄 한 줄. 절대 문장 끝에 이어 쓰거나 문단 안에 포함하지 마세요.
8. 마무리: 반드시 2문단(최소 150자)으로 작성하세요. 첫 문단은 오늘 소개한 정보에 대한 공감이나 에디터 소감, 두 번째 문단은 독자가 바로 확인하고 싶어지는 행동 격려 문장.
9. 이미지 마크다운 삽입 금지. 구분선(---, ***, ___) 절대 금지.
10. AI 티 나는 표현 절대 금지: 결론적으로/다양한/인상적인/포착한/정답/한마디로/종합하면/요약하면

[출력 구조 예시 — 반드시 이 형식 준수]
서론 문단 1
...
마무리 첫 문단 (공감/소감, 2~3문장)

마무리 두 번째 문단 (행동 격려, 2~3문장)

[출력 형식]
- 마크다운 본문만 출력 (frontmatter 없이)
- 서론 → 항목별 섹션(소제목+설명+링크) → 마무리(2문단) 순서
- 총 1000~1600자 내외
...
async function prepareCurationRequest(category, todayISO, postsDir, existingSlugs, requestIndex) {
  ...
  const topIdSets = loadTopIdSets();
  const ssgEligibleIds = topIdSets[category] || new Set();
