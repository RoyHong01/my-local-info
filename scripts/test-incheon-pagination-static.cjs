...
async function getCount(page) {
  return page.locator('[data-testid^="incheon-card-"]').count();
}
...
async function clickLoadMoreTo(page, targetCount) {
  for (let i = 0; i < 6; i += 1) {
    ...
    const button = page.getByTestId('incheon-load-more');
...
async function run() {
  ...
  try {
    await page.goto(`${BASE_URL}/incheon/`, { waitUntil: 'domcontentloaded' });
    await page.waitForSelector('[data-testid^="incheon-card-"]', { timeout: 10000 });
    ...
    await page.locator('[data-testid^="incheon-card-"]').nth(89).click();
    await page.waitForURL((url) => url.pathname.startsWith('/incheon/') && url.pathname !== '/incheon/', { timeout: 10000 });

    await page.goBack();
    await page.waitForURL((url) => url.pathname === '/incheon/', { timeout: 10000 });
    ...
    console.log('[PASS] Incheon static pagination checks passed');
  } finally {
    await browser.close();
  }
}
...
run().catch((error) => {
  console.error('[FAIL] Incheon static pagination checks failed');
