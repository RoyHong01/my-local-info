...
2) 훅 바로 아래 첫 2~3개 단락은 "감성형 도입"으로 작성:
  - 계절감/현장감/생활 맥락을 담은 문장 포함
  - 독자에게 질문하거나 공감 포인트를 던지는 문장 1개 이상 포함
  - 단순 사실 나열 금지, 감정 + 기대감 + 상황 묘사 포함
3) 사실 전달은 유지하되 문체는 블로그형으로 작성:
  - 예시 톤: "따스한 봄바람을 맞으며", "주말에 가볍게 떠나기 좋은", "지갑이 한결 가벼워지는"
  - 단, 과장/허위/확인되지 않은 정보는 절대 작성 금지
4) 추천 이유 3가지는 반드시 아래 형식으로 작성
  ### 1. 소제목
  (다음 줄에 설명 단락 2~4문장)
  ...
  ### 3. 소제목
  (다음 줄에 설명 단락 2~4문장)
5) "1. 소제목 설명"을 한 줄에 붙여 쓰지 말고, 소제목과 설명은 반드시 줄바꿈으로 분리
6) 전체 1000자 이상, 친근하고 따뜻한 블로그 톤, 신청/방문 방법 안내 포함
7) 본문 마지막 단락에는 독자가 바로 행동할 수 있는 한 줄 제안(예: 일정 확인/문의/신청)을 포함
