...
const GEMINI_MODEL = process.env.CHOICE_GEMINI_MODEL || process.env.GEMINI_MODEL || 'gemini-3.1-flash-lite';
...
- ⛔ 아래 표현은 제목에 사용 금지:
  ...
  (앞 항목들은 과거 생성물에서 과도하게 반복된 관용구이고,
   뒤 항목들은 판매 유도형 표현이다)
- ⛔ **아래는 최근 발행된 제목이다. 이 중 어느 것과도 동일하거나 유사한 제목을
  절대 만들지 마라.** 단어 몇 개만 바꾼 변형도 금지다.
  특히 문장 구조(어순·어미·질문 형태)가 같으면 실패로 간주한다.
${recentTitleBlock}
- 위 목록을 확인한 뒤, 그와 확실히 다른 각도로 제목을 만들어라.
